...
from ...core.text import clean_text, extract_json
from .models import PeriodAnalysis, PersonalAnalysis, PersonalPeriod, PersonalReport
...
def build_personal_report(
    ...
    target_messages = sorted(
        target_messages, key=lambda message: (message.created_at, message.message_id)
    )
    ...
    all_messages = sorted(
        all_messages, key=lambda message: (message.created_at, message.message_id)
    )
    ...
    for message in target_messages:
        ...
        if (
            previous is None
            or message.created_at - previous.created_at > SESSION_GAP
            or (
...
def build_detail_report(
    ...
    overview = build_personal_report(
        all_messages, target_messages, f"时段 {period_number} 详情"
    )
    ...
    transcript = sorted(
        context.union(overview.messages),
    ...
    period = PersonalPeriod(
        ...
        tuple(
            sorted(
                context, key=lambda message: (message.created_at, message.message_id)
            )
        ),
    ...
    return PersonalReport(
        ...
        overview.mode,
        True,
        tuple(transcript[(page - 1) * DETAIL_PAGE_SIZE : page * DETAIL_PAGE_SIZE]),
        page,
        page_count,
        period_number,
    )
...
def _sample(messages: tuple[ChatMessage, ...], limit: int) -> list[ChatMessage]:
    ...
    return [
        messages[round(index * (len(messages) - 1) / (limit - 1))]
        ...
        for message, role in sorted(
            ...
            + [
                (
                    message,
                    "目标成员时段外发言"
                    if message.sender_id == report.target_id
                    else "群内邻近发言",
                )
            ...
            if identity not in speaker_ids:
                ...
                members[speaker] = {
                    "sender_id": clean_text(message.sender_id, 80),
                    "name": clean_text(message.sender_name, 40),
                }
            transcript.append(
                [
                    role,
                    speaker_ids[identity],
                    dt.datetime.fromtimestamp(message.created_at).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    clean_text(message.text, 240 if report.detailed else 100),
                ]
            )
        periods.append(
            {
                "id": index,
                "time": time_range(period.start, period.end),
                "target_count": len(period.messages),
                "context_count": len(period.context),
                "session_count": period.session_count,
                "records": transcript,
            }
        )
    data = {
        "target_id": clean_text(report.target_id, 80),
        "target_name": clean_text(report.target_name, 40),
        "mode": report.mode,
        "target_count": len(report.messages),
        "members": members,
        "record_columns": ["role", "speaker", "time", "text"],
        "periods": periods,
    }
    return (
...
def build_personal_fallback(
    report: PersonalReport, reason: str = ""
) -> PersonalAnalysis:
    periods = []
    for period in report.periods:
        excerpts = "；".join(
            f"“{clean_text(message.text, 65)}”"
            for message in _sample(period.messages, 2)
        )
        context = "；".join(
            f"{clean_text(message.sender_name, 20)}：{clean_text(message.text, 55)}"
            for message in _sample(period.context, 2)
        )
        periods.append(
            PeriodAnalysis(
                "发言摘录",
                f"共 {len(period.messages)} 条发言。摘录：{excerpts}",
                f"邻近记录摘录（关联未判断）：{context}"
                if context
                else "该时段没有记录到邻近上下文。",
            )
        )
...
def parse_personal_analysis(raw: str, report: PersonalReport) -> PersonalAnalysis:
    ...
    for entry in payload["periods"]:
        if (
            ...
            and 1 <= entry["id"] <= len(report.periods)
        ):
    ...
    for index, (period, default) in enumerate(zip(report.periods, fallback.periods), 1):
        ...
        quote = clean_text(entry.get("quote"), 80)
        if quote and not any(
            quote in clean_text(message.text, 2000) for message in period.messages
        ):
            quote = ""
        periods.append(
            PeriodAnalysis(
                clean_text(entry.get("title"), 24) or default.title,
                clean_text(entry.get("summary"), 450 if report.detailed else 180)
                or default.summary,
                clean_text(entry.get("context"), 450 if report.detailed else 180)
                or default.context,
                quote,
            )
        )
    return PersonalAnalysis(
        clean_text(payload.get("overview"), 300) or fallback.overview,
        tuple(periods),
...
def format_personal_text(report: PersonalReport, analysis: PersonalAnalysis) -> str:
    ...
    if report.detailed:
        ...
        for message in report.transcript:
            ...
            lines.append(
                f"[{dt.datetime.fromtimestamp(message.created_at):%m-%d %H:%M:%S}] {message.sender_name}（{label}）：{message.text}"
            )
    lines.append(
