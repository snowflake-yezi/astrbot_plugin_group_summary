...
from astrbot_plugin_group_summary.report import ReportRenderer, compute_stats
...
class AnalysisCacheTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.messages = [message("2", index, f"发言 {index}", message_id=str(index)) for index in range(200)]
        self.report = build_personal_report(self.messages, self.messages, "全部")
        self.response = types.SimpleNamespace(completion_text=json.dumps({
            "overview": "发言概览", "periods": [{"id": 1, "title": "话题", "summary": "发言分析", "context": "讨论背景"}],
        }))
        self.provider = types.SimpleNamespace(model="model-a", text_chat=AsyncMock(return_value=self.response))
    ...
    async def test_detail_pages_share_analysis_but_keep_distinct_transcripts(self):
        ...
        analysis = await self.plugin._analyze_personal(self.event, first)
        cached = await self.plugin._analyze_personal(self.event, second)
    ...
    async def test_unsampled_message_change_invalidates_analysis(self):
        await self.plugin._analyze_personal(self.event, self.report)
        ...
        self.assertEqual(build_personal_prompt(changed_report), build_personal_prompt(self.report))
        await self.plugin._analyze_personal(self.event, changed_report)
    ...
    async def test_new_context_invalidates_analysis(self):
        ...
        changed_report = build_personal_report(self.messages + [nearby], self.messages, "全部")
        await self.plugin._analyze_personal(self.event, changed_report)
        self.assertEqual(self.provider.text_chat.await_count, 2)
    ...
    async def test_cache_is_scoped_to_platform_group_requester_and_conversation(self):
        ...
        for event in (MentionEvent(group_id="200"), MentionEvent(sender_id="8")):
            await self.plugin._analyze_personal(event, self.report)
        platform = MentionEvent()
        platform.get_platform_id = lambda: "other-platform"
        await self.plugin._analyze_personal(platform, self.report)
        conversation = MentionEvent()
        conversation.unified_msg_origin = "fake:other-conversation"
        await self.plugin._analyze_personal(conversation, self.report)
    ...
    async def test_switching_model_or_provider_invalidates_analysis(self):
        ...
        await self.plugin._analyze_personal(self.event, self.report)
        self.assertEqual(self.provider.text_chat.await_count, 2)
        replacement = types.SimpleNamespace(model="model-b", text_chat=AsyncMock(return_value=self.response))
    ...
    async def test_cache_expires_and_evicts_least_recently_used_entries(self):
        self.plugin.ANALYSIS_CACHE_TTL = -1
        await self.plugin._analyze_personal(self.event, self.report)
        self.plugin.ANALYSIS_CACHE_TTL = 600
        await self.plugin._analyze_personal(self.event, self.report)
        self.assertEqual(self.provider.text_chat.await_count, 2)
        self.plugin.MAX_CACHED_ANALYSES = 2
        other = MentionEvent(sender_id="8")
        await self.plugin._analyze_personal(other, self.report)
        await self.plugin._analyze_personal(self.event, self.report)
        await self.plugin._analyze_personal(MentionEvent(sender_id="9"), self.report)
        self.assertEqual(len(self.plugin._analysis_cache), 2)
        await self.plugin._analyze_personal(self.event, self.report)
    ...
    async def test_model_timeout_cancels_request_and_fallback_is_not_cached(self):
        ...
        self.plugin.MODEL_TIMEOUT = 0.01
        self.provider.text_chat.side_effect = never_finishes
        fallback = await self.plugin._analyze_personal(self.event, self.report)
        self.assertEqual(fallback.source, "statistics")
        self.assertIn("超时", fallback.overview)
        self.assertTrue(cancelled.is_set())
        self.assertFalse(self.plugin._analysis_cache)
        self.provider.text_chat.side_effect = None
        recovered = await self.plugin._analyze_personal(self.event, self.report)
        self.assertEqual(recovered.source, "model")
        self.assertEqual(self.provider.text_chat.await_count, 2)

    async def test_invalid_model_response_is_not_cached(self):
        self.provider.text_chat.return_value = types.SimpleNamespace(completion_text="无效响应")
        fallback = await self.plugin._analyze_personal(self.event, self.report)
        self.assertEqual(fallback.source, "statistics")
        self.assertFalse(self.plugin._analysis_cache)
    ...
    async def test_group_summary_reuses_successful_analysis_and_limits_model_wait(self):
        self.provider.text_chat.return_value = types.SimpleNamespace(completion_text=json.dumps({
            "title": "聊天概览", "topics": [{"title": "话题", "summary": "讨论话题"}],
        }))
        stats = compute_stats(self.messages, dt.date(2026, 9, 1))
        first = await self.plugin._analyze(self.event, self.messages, stats)
        second = await self.plugin._analyze(self.event, self.messages, stats)
        self.assertIs(first, second)
        self.provider.text_chat.assert_awaited_once()
        self.plugin._analysis_cache.clear()
        self.plugin.MODEL_TIMEOUT = 0.01
        ...
        with self.assertRaises(asyncio.TimeoutError):
            await self.plugin._analyze(self.event, self.messages, stats)
        self.assertFalse(self.plugin._analysis_cache)
...
class HistoryBatchTests(unittest.IsolatedAsyncioTestCase):
    async def test_twenty_thousand_recent_records_use_twenty_queries(self):
        messages = [message("2", index, f"消息 {index}", message_id=str(index)) for index in range(20_001)]
        ...
        loaded = await GroupSummaryPlugin(context)._load_messages(MentionEvent())
    ...
    async def test_time_bounds_skip_text_processing_and_keep_exact_edges(self):
        messages = [message("2", index, f"消息 {index}", message_id=str(index)) for index in range(100)]
        plugin = GroupSummaryPlugin(FakeContext(rows_for(messages)))
        with patch.object(plugin, "_normalize_text", wraps=plugin._normalize_text) as normalize:
            loaded = await plugin._load_messages(MentionEvent(), bounds=(messages[40].created_at, messages[45].created_at))
...
class TextLayoutTests(unittest.TestCase):
    ...
    def test_wrapping_preserves_content_and_respects_font_width(self):
        samples = ("中文排版测试 English AVWA 09:30 123 " * 5, "cafe\u0301 e\u0301 hello 世界 " * 4)
        for text in samples:
            for width in (60, 130, 1088):
                with self.subTest(width=width, text=text[:20]):
                    ...
                    self.assertTrue(all(self.renderer.draw.textlength(line, font=self.renderer.font_body) <= width for line in lines))
                    for previous, following in zip(lines, lines[1:]):
                        self.assertGreater(self.renderer.draw.textlength(previous + following[0], font=self.renderer.font_body), width)

    def test_newlines_and_truncation_keep_layout_bounded(self):
        self.assertEqual(self.renderer._wrap("一\n\n二\n", self.renderer.font_body, 200), ["一", "", "二", ""])
        lines = self.renderer._wrap("较长的中文句子" * 20, self.renderer.font_body, 130, max_lines=2)
        self.assertEqual(len(lines), 2)
        self.assertTrue(lines[-1].endswith("..."))
        self.assertTrue(all(self.renderer.draw.textlength(line, font=self.renderer.font_body) <= 130 for line in lines))

    def test_long_text_uses_fewer_font_measurements(self):
        text = "需要快速排版的长消息 English words 09:30。" * 50
        with patch.object(self.renderer.draw, "textlength", wraps=self.renderer.draw.textlength) as measure:
