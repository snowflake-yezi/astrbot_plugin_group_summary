...
async def benchmark() -> None:
    messages = [
        message(
            "2" if index % 5 == 0 else "3",
            (index // 1600) * 120 + (index % 1600) // 60,
            "讨论版本配置和排查结果。" * 12,
    ...
    loaded = await plugin.history.load(event)
    ...
    for page in (1, 2):
        ...
        await plugin.personal_handler.service.analyze(event, detail)
    ...
    long_messages = [
        message(
            "2",
            index,
            "中文长消息排版测试，含时间 09:00 和 English words。" * 35,
            message_id=str(index),
        )
        ...
        json.dumps(
            {
                ...
                "simulated_query_delay_ms": 10,
                "load_seconds": round(load_seconds, 3),
                "build_seconds": round(build_seconds, 3),
                "overview_prompt_chars": prompt_chars,
                "overview_render_seconds": round(render_seconds, 3),
                "overview_image_bytes": image_bytes,
                "two_detail_pages_model_calls": provider.text_chat.await_count,
